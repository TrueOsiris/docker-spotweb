...
if [ -f /config/$(echo $initfile) ]; then
    ...
    echo "<? header('Location: /spotweb/install.php'); ?>" > /www/index.php
    echo "> Creating /config/dbsettings.inc.php.sample ..."
    touch /config/dbsettings.inc.php.sample 2>/dev/null
    echo "> Changing ownership of /config/dbsettings.inc.php.sample ..."
    chown www-data:www-data /config/dbsettings.inc.php.sample 2>&1
